...
nondet(opposite/2) :- !.
opposite(integer, boolean).
opposite(boolean, integer).
...
output(NLvl, ELvl, Maj, Env, Ret, Out) :-
    expr(NLvl, ELvl, Maj, Ret, Env, boolean, Out).
...
input(NLvl, ELvl, Maj, Env, Ret, Ref) :-
    Ref = ref(_, _),
    expr(NLvl, ELvl, Maj, Ret, Env, boolean, Ref).
...
stmt(_, _, _, _, false, Env, Env, exit).
stmt(NLvl, ELvl, Maj, _, Ret, Env, Env, exit_when(Cond)) :-
    expr(NLvl, ELvl, Maj, Ret, Env, integer, Cond).
...
stmt(NLvl, ELvl, Maj, _, Ret, Env, Env, call(Name, Args)) :-
    ...
    member(Name-procedure(PTypes), E), !,
    maplist(opposite, PTypes, Types),
...
stmt(NLvl, ELvl, _, _, false, Env, [Func | Env], function(Type, Name, Args, Body)) :-
    ...
    maplist(expr_type, Types),
    append([Func | Args], Env, Env1),
    stmt(NLvl, ELvl, function(Type), none, false, Env1, _, Body).
stmt(_, _, _, _, true, Env, Env, return).
stmt(NLvl, ELvl, Major, _, true, Env, Env, return_with(Expr)) :-
    expr_type(Type),
    expr(NLvl, ELvl, Major, _, Env, Type, Expr).
...
expr(NLvl, ELvl, Maj, Ret, Env, Type, op(Op, Expr1, Expr2)) :-
    ...
    opposite(OpType, OpType1),
    expr(NLvl, ELvl1, Maj, Ret1, Env, OpType, Expr1),
    expr(NLvl, ELvl1, Maj, Ret2, Env, OpType1, Expr2).
expr(NLvl, ELvl, Maj, Ret, Env, Type, ref(Name, Args)) :-
    random_permutation(Env, E),
    member(Name-FullType, E),
    decl_type(Type, _, FullType), !,
    random_between(0, 2, N),
    length(Rets, N),
    length(Args, N),
    rets(Rets, Ret),
    findall(X, (
        member(R, Rets),
        elvl(ELvl, ELvl1),
        expr(NLvl, ELvl1, Maj, R, Env, boolean, X)
    ), Args).
expr(NLvl, ELvl, Maj, Ret, Env, Type, yields(Stmt, Expr)) :-
    nlvl(NLvl, NLvl1),
    nlvl(ELvl, ELvl1),
    rets([Ret1, Ret2], Ret),
    stmt(NLvl1, ELvl1, Maj, none, Ret1, Env, _, Stmt),
    opposite(Type, Type1),
    expr(NLvl1, ELvl1, Maj, Ret2, Env, Type1, Expr).
expr(NLvl, ELvl, Maj, Ret, Env, Type, call(Name, Args)) :-
    elvl(ELvl, ELvl1),
    random_permutation(Env, E),
    member(Name-function(Type, FTypes), E), !,
    maplist(opposite, FTypes, Types),
    same_length(Types, Envs),
    same_length(Types, Rets),
    rets(Rets, Ret),
    maplist(=(Env), Envs),
