...
exec_test () {
    ...
    echo -n "$file" ::
    java -jar $WHERE/dist/compiler488.jar --semantic-only >/dev/null $file 2>$ERRFILE
    if [ -s $ERRFILE ]
    then
...
A3_CASES="$(find ./tests/a3tests -name '*.488' | sort -V)"
...
echo --------------------------------------------
echo Checking A3 files
echo --------------------------------------------
for file in $A3_CASES; do exec_test "$file"; done
