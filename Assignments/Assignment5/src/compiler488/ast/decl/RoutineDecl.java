...
public class RoutineDecl extends Declaration {
	...
	public StmtSemantics checkSemantics(
		...
		StmtSemantics sem = checkBody(table, errors);
		if (!sem.isReturning() && type != null)
	...
	private StmtSemantics checkBody(SymbolTable table, ArrayList<String> errors) {
	    MajorScope majorScope = type == null ? new MajorScope.Procedure() : new MajorScope.Function(type);
	    MinorScope minorScope = new MinorScope.None();
	    boolean returning = false;
	    if (body.getBody() != null) {
		    for (Stmt stmt : body.getBody())
