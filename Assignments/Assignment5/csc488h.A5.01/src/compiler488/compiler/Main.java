...
public class Main {
  ...
  /** FIXME, Debug option -- semantic check only */
  public static boolean semanticOnly   = false ;
   ...
   */
	...
	if (length > 0)
	  try{		// catch arrayOutOfBoundsException for bad argument list
		...
		else if(arguments[i].equals("--semantic-only")) {
		    semanticOnly = true;
		}
  ...
  private static void semanticAnalysis(Program programAST) {
		try {
			...
			programAST.checkSemantics(new MajorScope.Top(), new MinorScope.None(), new SymbolTable(), errors);
			errorOccurred = !errors.isEmpty();
  ...
  private static void generateCode( Program  programAST ) {
	...
	try{
	   ...
	   // codeGen.doIt( programAST );
		...
		compiler488.codegen.ir.Generator gen = new compiler488.codegen.ir.Generator();
		gen.compile_ir(programAST);
		gen.commit_binary();
	}
        catch( Exception e)
	    {
	...
	try {
	    Parser p = new Parser(new Lexer(new FileReader(sourceFileName )));
            if(  traceSyntax )
	         ...
	         Symbol tempParserResult = p.parse();
	         if(tempParserResult==null) {
	             errorOccurred = true ;
	             System.err.println("Error: Parser returned a null symbol");
	             return ;
	         }
	         parserResult = tempParserResult.value;
		}
	         // give up if we do not have a program
	         if (parserResult==null || !(parserResult instanceof Program)) {
	             errorOccurred = true ;
	...
	/* process user options and arguments */
        ...
        */
		...
		for( ; sourceFileIndex < argv.length ; sourceFileIndex++ ) {
			...
			if(semanticOnly) continue; // FIXME
