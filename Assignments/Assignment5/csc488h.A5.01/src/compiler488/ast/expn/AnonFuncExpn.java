package compiler488.ast.expn;

import java.util.*;

import compiler488.ast.ASTList;
import compiler488.ast.PrettyPrinter;
import compiler488.ast.stmt.Stmt;
import compiler488.semantics.*;
import compiler488.symbol.*;

/**
 * Represents the parameters and instructions associated with a function or
 * procedure.
 */
public class AnonFuncExpn extends Expn {
    /** Execute these statements. */
    private ASTList<Stmt> body;

    /** The expression whose value is yielded. */
    private Expn expn;

    public AnonFuncExpn(int line, int column, ASTList<Stmt> body, Expn expn) {
        super(line, column);

        this.body = body;
        this.expn = expn;
    }

    public ASTList<Stmt> getBody() {
        return body;
    }

    public Expn getExpn() {
        return expn;
    }

    public void prettyPrint(PrettyPrinter p) {
        p.println("{");
        p.enterBlock();

        body.prettyPrintNewlines(p);

        p.print("yields ");
        expn.prettyPrint(p);
        p.newline();

        p.exitBlock();
        p.println("}");
    }

	@Override
	public ExpnSemantics checkSemantics(MajorScope scope, SymbolTable table, ArrayList<String> errors) {
		table.pushScope();
		
		boolean returning = false;
		for (Stmt stmt : body)
			returning |= stmt.checkSemantics(scope, new MinorScope.None(), table, errors).isReturning();
		
		ExpnSemantics sem = expn.checkSemantics(scope, table, errors);
		returning |= sem.isReturning();
		
		table.popScope();
		
		return new ExpnSemantics(returning, sem.getType());
	}
}
